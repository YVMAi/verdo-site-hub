...
export const CompactCleaningDataEntry: React.FC<CompactCleaningDataEntryProps> = ({
  data,
  onDataChange
}) => {
      ...
      <div className="bg-verdo-navy px-3 py-2 text-white font-medium text-sm flex justify-between items-center">
        ...
        <div className="flex gap-4">
          ...
          <div className="flex flex-col items-center">
            <Button onClick={handleSave} variant="outline" size="sm" className="bg-transparent border-white text-white hover:bg-white/10 w-8 h-8 p-0">
            ...
            <table className="w-full text-xs border-collapse">
              <thead className="sticky top-0">
                <tr className="bg-verdo-navy text-white">
                  ...
                  {data.blocks.map(block => <th key={block.id} className="px-2 py-1 text-center font-medium border border-gray-300" colSpan={block.inverters.length}>
                      {block.name}
                    </th>)}
                ...
                <tr className="bg-verdo-navy text-white">
                  ...
                  {data.blocks.map(block => block.inverters.map(inverter => <th key={`${block.id}-${inverter.id}`} className={cn("px-2 py-1 text-center font-medium border border-gray-300", getColumnWidth(`${block.id}-${inverter.id}`))}>
                        {inverter.id}
                      </th>))}
              ...
              <tbody>
                <tr className="bg-gray-200">
                  ...
                  {data.blocks.map(block => block.inverters.map(inverter => <td key={`total-${block.id}-${inverter.id}`} className={cn("px-2 py-1 text-center border border-gray-300", getColumnWidth(`${block.id}-${inverter.id}`))}>
                ...
                <tr className="bg-gray-200">
                  ...
                  {data.blocks.map(block => block.inverters.map(inverter => <td key={`percent-${block.id}-${inverter.id}`} className={cn("px-2 py-1 text-center border border-gray-300", getColumnWidth(`${block.id}-${inverter.id}`))}>
                ...
                <tr className="bg-blue-100">
                  <td className="px-2 py-1 font-medium border border-gray-300">
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant={"outline"} className={cn("h-6 px-2 justify-start text-left font-normal text-xs", "bg-white border border-gray-300 text-gray-900 hover:bg-gray-50")}>
                  ...
                  {data.blocks.map(block => block.inverters.map(inverter => {
                const key = `${block.id}-${inverter.id}`;
                return <td key={`input-${key}`} className={cn("px-2 py-1 text-center border border-gray-300", getColumnWidth(key))}>
                          <input type="number" className="w-full h-6 text-center text-xs border-0 bg-transparent focus:bg-white" value={inputValues[key] || ""} onChange={e => handleInputChange(key, e.target.value)} />
                        </td>;
              }))}
                  <td className="px-2 py-1 border border-gray-300">
                    <textarea className="w-full h-6 text-xs border-0 bg-transparent focus:bg-white resize-none" style={{
                  minHeight: '24px',
                  whiteSpace: 'pre-wrap',
                  wordWrap: 'break-word'
