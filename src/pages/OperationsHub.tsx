...
import { Scissors, Droplets, Search, Leaf, CheckCircle2, Clock, BarChart3, Circle, Lock, Wrench, Package, FileText } from "lucide-react";
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="min-w-[250px]">
              <ClientSiteSelector selectedClient={selectedClient} selectedSite={selectedSite} onSiteChange={setSelectedSite} />
            </div>
          ...
          <div className="border-b border-gray-200 bg-white px-6 py-2">
            <ScrollArea className="w-full whitespace-nowrap">
              <TabsList className="inline-flex h-auto p-0 gap-0 bg-transparent w-max">
                ...
                return <TabsTrigger key={operation.id} value={operation.id} disabled={!operation.enabled} className={`
                        flex items-center gap-2 h-10 px-4 border-b-3 font-medium text-sm whitespace-nowrap transition-all duration-200
                        ${isActive ? 'bg-slate-800 text-white border-slate-900 shadow-sm' : 'bg-transparent text-gray-600 border-transparent hover:bg-gray-50 hover:text-gray-800'}
