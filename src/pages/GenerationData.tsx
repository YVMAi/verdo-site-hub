  ...
  return <div className="min-h-screen w-full flex flex-col bg-white">
      ...
      <div className="bg-[hsl(var(--verdo-navy))] text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Bulk Upload */}
          <div className="flex items-center gap-2">
            <BulkUploadModal selectedSite={selectedSite} activeTab={activeTab} onUpload={handleBulkUpload} />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white overflow-hidden">
        <Tabs value={activeTab} onValueChange={value => setActiveTab(value as TabType)} className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-auto">
            {tabConfigs.map(tab => <TabsContent key={tab.id} value={tab.id} className="h-full m-0 p-6">
                {/* Data Entry Table */}
                <div className="mb-8">
                  
                  <DataEntryTable site={selectedSite} activeTab={activeTab} selectedDate={selectedDate} />
                </div>

                {/* Historic Data Table */}
                <div>
                  
                  <HistoricDataTable site={selectedSite} activeTab={activeTab} allowedEditDays={selectedClient?.allowedEditDays || 30} />
                </div>
              </TabsContent>)}

            {/* Info Panel */}
            {(!selectedClient || !selectedSite) && <div className="bg-muted/30 border border-dashed rounded-lg p-8 text-center m-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground max-w-md mx-auto">
                      {!selectedClient ? "Select a client from the sidebar to begin entering and viewing generation data." : "Select a site from the dropdown above to begin entering and viewing generation data."}
                    </p>
                  </div>
                </div>
              </div>}
