...
export default function ClientManagement() {
  ...
  const [clientStatuses, setClientStatuses] = useState<ClientStatus[]>(mockClients.map(client => ({
    id: client.id,
    enabled: true,
  ...
  const handleSync = async () => {
    ...
    setClientStatuses(prev => prev.map(status => ({
      ...status,
      lastSynced: new Date().toISOString()
    })));
    setSyncing(false);
  ...
  const toggleClientStatus = (clientId: string) => {
    setClientStatuses(prev => prev.map(status => status.id === clientId ? {
      ...status,
  ...
  return <div className="min-h-screen w-full flex flex-col bg-white">
      ...
      <div className="bg-[hsl(var(--verdo-navy))] text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {/* Sync Button */}
          <Button onClick={handleSync} disabled={syncing} className="bg-white/10 hover:bg-white/20 border-white/20 text-white h-8">
            ...
            <Table>
              ...
              <TableBody>
                ...
                return <TableRow key={client.id}>
                      ...
                      <TableCell>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${status?.enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Switch checked={status?.enabled || false} onCheckedChange={() => toggleClientStatus(client.id)} />
                        </div>
                      </TableCell>
                    </TableRow>;
              })}
